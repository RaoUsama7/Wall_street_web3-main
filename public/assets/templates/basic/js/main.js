(function ($) {
  ...
  $(document).ready(function () {
    if (window.location.href.includes('/us-market')) {
      let checkInterval = setInterval(function () {
        const $rows = $('#market-list-body tr');
        if ($rows.length > 0) {
          $rows.each(function () {
            const pairName = $(this).find('.customer__name').text().trim();
            if (pairName !== 'US100_USDT') {
              $(this).hide();
            } else {
              $(this).show();
            }
        ...
        $('.table-wrapper .table-wrapper__item').hide();
      }, 1000);
    } else {
      console.log("ℹ️ Not on /us-market page, script skipped.");
    }
  ...
  // trade btn js start here 

  $(document).on('click', '.allTradeBtn', function (e) {
    e.stopPropagation();
    const $tradeBtnWrapper = $(this).closest('.trade-btn-wrapper');
    $('.trade-list').not($tradeBtnWrapper.find('.trade-list')).removeClass('show');
    $tradeBtnWrapper.find('.trade-list').toggleClass('show');
  });

  $(document).on('click', function (e) {
    if (!$(e.target).closest('.trade-btn-wrapper').length) {
      $('.trade-list').removeClass('show');
    }
  });

  // trade btn js end here 
