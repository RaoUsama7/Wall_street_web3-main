...
class BinaryTradeController extends Controller {
    public function binary($id = 0) {
        ...
        if (empty($durations) || !is_array($durations)) {
            $durations = [60]; // Default to 60 seconds if no durations set
        }
    ...
    public function tradeTabClose($id, $firstCoinId) {
        ...
        session()->put('coinPairsBinary', $tradeTabs);
        $binaryDurations = $activeCoin->binary_trade_duration ?? [];
        if (is_object($binaryDurations)) {
            $binaryDurations = (array) $binaryDurations;
        }
        if (empty($binaryDurations) || !is_array($binaryDurations)) {
            $binaryDurations = [60]; // Default to 60 seconds
        }
        $firstDuration = convertToMinutesSeconds($binaryDurations[0] ?? 60) ?? '0:00';
        $durations     = '';
        foreach ($binaryDurations as $duration) {
