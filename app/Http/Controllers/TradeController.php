...
class TradeController extends Controller {
    public function trade($symbol = null) {
        ...
        // Preserve original symbol for display
        $originalPairSymbol = $pair->symbol;

        // Extract base symbol (remove currency suffix like USDC, USD, USDT, etc.)
        $baseSymbol = null;
        if (isset($pair->coin) && isset($pair->coin->symbol) && !empty($pair->coin->symbol)) {
            $baseSymbol = strtoupper(trim($pair->coin->symbol));
        }
        
        // If coin symbol not available, extract from pair symbol
        if (empty($baseSymbol)) {
            $pairSymbol = strtoupper(str_replace(['/', '_'], '', $originalPairSymbol));
            // Remove common currency suffixes
            $currencySuffixes = ['USD', 'USDC', 'USDT', 'BTC', 'ETH', 'BNB', 'BUSD'];
            foreach ($currencySuffixes as $suffix) {
                if (strlen($pairSymbol) > strlen($suffix) && strtoupper(substr($pairSymbol, -strlen($suffix))) === $suffix) {
                    $baseSymbol = substr($pairSymbol, 0, -strlen($suffix));
                    break;
                }
            }
            if (empty($baseSymbol)) {
                $baseSymbol = $pairSymbol;
            }
        }
        
        // Set symbols for display and chart usage
        $pair->display_symbol = $originalPairSymbol;
        $pair->chart_symbol   = strtoupper(trim($baseSymbol));
    ...
    public function orderBook($symbol = null) {
        ...
        if ($orderType == 'all' || $orderType == 'sell') {
            $sellSideOrders = (clone $query)->sellSideOrder()->take(13)->get();
        }
        if ($orderType == 'all' || $orderType == 'buy') {
            $buySideOrders = (clone $query)->buySideOrder()->take(13)->get();
        }
    ...
    public function pairs() {
        ...
        $favoritePairId = FavoritePair::where('user_id', auth()->id() ?? 0)->where('pair_id', '>=', 0)->pluck('pair_id')->toArray();
