...
class FutureTradeOrderController extends Controller
{
    ...
    public function order(Request $request)
    {
        ...
        // If FutureTradeConfig doesn't exist, try to create it if the pair exists
        if (!$futureTradeConfig) {
            $pair = \App\Models\CoinPair::active()->where('id', $request->pair_id)->first();
            if ($pair) {
                // Create FutureTradeConfig for the pair
                $futureTradeConfig = new FutureTradeConfig();
                $futureTradeConfig->pair_id = $pair->id;
                $futureTradeConfig->leverage = 10; // Default leverage
                $futureTradeConfig->min_buy_amount = 0.01;
                $futureTradeConfig->max_buy_amount = -1; // No limit
                $futureTradeConfig->min_sell_amount = 0.01;
                $futureTradeConfig->max_sell_amount = -1; // No limit
                $futureTradeConfig->buy_charge = 0;
                $futureTradeConfig->sell_charge = 0;
                $futureTradeConfig->maintenance_margin_rate = 0.01; // 1% maintenance margin
                $futureTradeConfig->status = Status::ENABLE;
