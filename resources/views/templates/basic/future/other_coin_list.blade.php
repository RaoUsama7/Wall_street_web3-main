            ...
            <tbody class="order-list-body">
                @foreach ($otherPairs as $otherPair)
                    <tr class="{{ in_array($otherPair->id, $favoritePairs) ? '' : 'd-none' }}">
                        <td>
                            <div class="trading-dropdown-button d-flex">
                                ...
                                <a href="{{ route('future.trade', $otherPair->coinPair->symbol) }}" class="d-block trading-dropdown-button-title">
                                    {{ $otherPair->coinPair->symbol }}
                                    <span class="d-block fs-12 vol">
                                        @lang('Mar. Cap')
                                        {{ formatNumber(optional($otherPair->coinPair->marketData)->market_cap ?? 0) }}
                                    </span>
                                </a>
                                <div>
                        ...
                        <td>
                            <span>{{ showAmount(optional($otherPair->coinPair->marketData)->price ?? 0, currencyFormat: false) }}</span>
                        </td>
                        <td>
                            @php
                                $marketData = optional($otherPair->coinPair->marketData);
                            @endphp
                            <span class="{{ @$marketData->html_classes->percent_change_24h }}">{{ getAmount($marketData->percent_change_24h ?? 0, 2) }}%</span>
                        </td>
                    </tr>
                @endforeach
            </tbody>
